...
import { ExecutableTarget } from '../src/types.js';
...
describe('Performance Tests', () => {
  ...
  describe('Build Debouncing', () => {
    ...
    it('should handle different settling delays per target', async () => {
      ...
      expect(fastBuilder?.build).toHaveBeenCalledWith(['fast/file.ts']);
      ...
      expect(slowBuilder?.build).toHaveBeenCalledWith(['slow/file.ts']);
    });
  ...
  describe.skip('Build Queue Management', () => {
    it('should queue builds during active build', { timeout: 10000 }, async () => {
      // Make build take time
      let buildResolve: () => void;
      vi.mocked(testBuilder?.build).mockImplementation(() => 
        new Promise(resolve => {
      ...
      changeHandler([{ name: 'src/file2.ts', exists: true, type: 'f' }]);
      changeHandler([{ name: 'src/file3.ts', exists: true, type: 'f' }]);
      vi.advanceTimersByTime(110);

      // Should not trigger another build yet (still building)
      expect(testBuilder?.build).toHaveBeenCalledTimes(1);
      ...
      // Advance time to trigger queued build
      vi.advanceTimersByTime(110);
      ...
      // Should build again with queued files
      expect(testBuilder?.build).toHaveBeenCalledTimes(2);
      expect(testBuilder?.build).toHaveBeenLastCalledWith([
        'src/file2.ts',
        'src/file3.ts',
      ]);
    });

    it('should not create unbounded build queue', { timeout: 10000 }, async () => {
      // Make builds slow
      let buildResolvers: (() => void)[] = [];
      let buildCount = 0;
      vi.mocked(testBuilder?.build).mockImplementation(() => 
        new Promise(resolve => {
          const index = buildCount++;
          buildResolvers[index] = () => resolve({
            status: 'success',
            targetName: 'test',
            timestamp: new Date().toISOString(),
            duration: 1000,
      ...
      // Complete any pending builds
      buildResolvers.forEach(resolver => resolver?.());
      await Promise.resolve();

      // Should coalesce changes, not create 1000 builds
      const buildCalls = vi.mocked(testBuilder?.build).mock.calls.length;
      expect(buildCalls).toBeLessThan(10); // Reasonable number of builds
    });
  });
  ...
  describe('Resource Cleanup', () => {
    ...
    it('should handle cleanup errors gracefully', async () => {
      ...
      vi.mocked(harness.watchmanClient.disconnect).mockRejectedValue(new Error('Disconnect failed'));
