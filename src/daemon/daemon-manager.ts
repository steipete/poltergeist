...
export class DaemonManager {
  ...
  private getDaemonInfoPath(projectPath: string): string {
    const hash = FileSystemUtils.getProjectHash(projectPath);
    return join(FileSystemUtils.getStateDirectory(), `${hash}-daemon.json`);
  }
  ...
  private getLogFilePath(projectPath: string): string {
    const hash = FileSystemUtils.getProjectHash(projectPath);
    return join(FileSystemUtils.getStateDirectory(), `${hash}-daemon.log`);
  }
