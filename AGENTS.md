...
- Use the command wrappers provided by the workspace (`./runner …`, `pnpm mcp:*`, etc.). Skip them only for trivial read-only shell commands if that’s explicitly allowed.
...
- When editing shared guardrail scripts (runners, browser tools, etc.), mirror the same change back into the `agent-scripts` folder so the canonical copy stays current.
...
- Maintain strict typing—avoid `any`, prefer utility helpers already provided by the repo, and keep shared guardrail scripts (runner and browser helpers) consistent by syncing back to `agent-scripts` when they change.
