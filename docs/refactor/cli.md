...
- 🔸 Move command registration to declarative descriptors (name, description, options, handler factory) consumed by both Commander wiring and help formatter. (Registry + help now share descriptors and option metadata; Commander wiring still manual to avoid flag conflicts—needs safer per-command adoption.)
- ✅ Introduce shared option schemas for common flags (config, target, log-level) to keep descriptions/aliases aligned and auto-generate help; registry now documents all command options for help/tests.
